...
local CN_soundDescriptions = {
  ...
  ["CHAT_MSG_ADDON"] =
  "When an addon sends a message not visible to the player that is NOT logged on Blizzard's servers (such as the feature in DBM that will check if other players are running the addon).",
  ["CHAT_MSG_ADDON_LOGGED"] =
  "When an addon sends a message not visible to the player that IS logged on Blizzard's servers (such as an RP addon sharing your characters bio)",
  ["CHAT_MSG_AFK"] = "When you receive an AFK auto response",
  ["CHAT_MSG_BG_SYSTEM_ALLIANCE"] =
  "When an alliance action is sent to chat in BGs, e.g. assaulting a graveyard or capture point, or picking up a flag.",
  ...
  "When you receive an auto-response for the battle.net whisper target being offline",
  ...
  "When something changes in the channel, such as a user being kicked or moderation being enabled",
  ...
  "When you gain XP from a creature or a quest. (does not activate if you kill a creature and do not get XP from it)",
  ...
  "When you receive a message in instance chat from the instance leader, such as when you're in a match-made dungeon or raid.",
  ["CHAT_MSG_LOOT"] =
  "When loot text is sent to your chat, such as when somebody receives an item, rolls for an item, or creates an item.",
...
local function changeSettings(self)
  ...
  configUI.panel.selected.soundDropDown.Text:SetText(type)
...
local function checkedTestSound(self)
  return (self.value == soundDropDownDictionary[configUI.panel.selected.soundDropDown.Text:GetText()])
end
...
local dropdownData = {
  ["NormalMessages"] = {
    ...
    {
      text = "BN_WHISPER_INFORM",
      value = "CHAT_MSG_BN_WHISPER_INFORM",
      func = changeSettings,
      checked = checkedTest,
    },
    {
      text = "BN_WHISPER_PLAYER_OFFLINE",
      value = "CHAT_MSG_BN_WHISPER_PLAYER_OFFLINE",
      func = changeSettings,
      checked = checkedTest,
    },
    { text = "CHANNEL",       value = "CHAT_MSG_CHANNEL",       func = changeSettings, checked = checkedTest },
    {
      text = "COMMUNITIES_CHANNEL",
      value = "CHAT_MSG_COMMUNITIES_CHANNEL",
      func = changeSettings,
      checked = checkedTest,
    },
    ...
    {
      text = "INSTANCE_CHAT_LEADER",
      value = "CHAT_MSG_INSTANCE_CHAT_LEADER",
      func = changeSettings,
      checked = checkedTest,
    },
  ...
  ["SystemAndGameMessages"] = {
    ...
    { text = "ADDON_LOGGED", value = "CHAT_MSG_ADDON_LOGGED", func = changeSettings, checked = checkedTest },
    {
      ...
      value = "CHAT_MSG_BG_SYSTEM_ALLIANCE",
      func = changeSettings,
      checked = checkedTest,
    },
    { text = "BG_SYSTEM_HORDE", value = "CHAT_MSG_BG_SYSTEM_HORDE", func = changeSettings, checked = checkedTest },
    {
      ...
      value = "CHAT_MSG_BG_SYSTEM_NEUTRAL",
      func = changeSettings,
      checked = checkedTest,
    },
    {
      text = "COMBAT_FACTION_CHANGE",
      value = "CHAT_MSG_COMBAT_FACTION_CHANGE",
      func = changeSettings,
      checked = checkedTest,
    },
    {
      text = "COMBAT_HONOR_GAIN",
      value = "CHAT_MSG_COMBAT_HONOR_GAIN",
      func = changeSettings,
      checked = checkedTest,
    },
    {
      text = "COMBAT_MISC_INFO",
      value = "CHAT_MSG_COMBAT_MISC_INFO",
      func = changeSettings,
      checked = checkedTest,
    },
  ...
  ["Social"] = {
    ...
    {
      text = "CHANNEL_NOTICE_USER",
      value = "CHAT_MSG_CHANNEL_NOTICE_USER",
      func = changeSettings,
      checked = checkedTest,
    },
    {
      text = "GUILD_ACHIEVEMENT",
      value = "CHAT_MSG_GUILD_ACHIEVEMENT",
      func = changeSettings,
      checked = checkedTest,
    },
    {
      text = "GUILD_ITEM_LOOTED",
      value = "CHAT_MSG_GUILD_ITEM_LOOTED",
      func = changeSettings,
      checked = checkedTest,
    },
  },
  ["NPCChatter"] = {
    ...
    {
      text = "RAID_BOSS_WHISPER",
      value = "CHAT_MSG_RAID_BOSS_WHISPER",
      func = changeSettings,
      checked = checkedTest,
    },
  },
  ["Sounds"] = {
    ...
    {
      text = "Guild",
      value = "Interface\\AddOns\\ChatNotifications\\guild.ogg",
      func = changeSound,
      checked = checkedTestSound,
    },
    {
      text = "Party",
      value = "Interface\\AddOns\\ChatNotifications\\party.ogg",
      func = changeSound,
      checked = checkedTestSound,
    },
    {
      text = "Wheep",
      value = "Interface\\AddOns\\ChatNotifications\\wheep.ogg",
      func = changeSound,
      checked = checkedTestSound,
    },
    {
      text = "Whisper",
      value = "Interface\\AddOns\\ChatNotifications\\whisper.ogg",
      func = changeSound,
      checked = checkedTestSound,
    },
    {
      text = "Woopwoop",
      value = "Interface\\AddOns\\ChatNotifications\\woopwoop.ogg",
      func = changeSound,
      checked = checkedTestSound,
    },
  },
...
local function saveButton_OnClick()
  ...
  local sound = configUI.panel.selected.soundDropDown.Text:GetText()
...
local function createDropDown(text, name, location, offsetx, offsety, size, init, point1, point2)
  local dropDown = CreateFrame("FRAME", "CN_Config_" .. name, location, "UIDropDownMenuTemplate")
  dropDown.selected = init[1].value
  function dropDown:SetValue(newValue)
    dropDown.selected = newValue.value
    newValue.func(newValue)
  end

  dropDown:SetPoint(point1 or "TOP", location, point2 or "TOPLEFT", offsetx, offsety)
  UIDropDownMenu_SetWidth(dropDown, size)
  ...
  UIDropDownMenu_Initialize(dropDown, function(self, level, menuList)
    ...
    for k, v in pairs(init) do
      info.text, info.checked = v.text, dropDown.selected == v.value
      info.arg1 = v
...
configUI.panel.normalDropDown =
    createDropDown("Normal Chat", "normalDropDown", configUI.panel, 59, -15, 120, dropdownData.NormalMessages)

configUI.panel.systemDropDown = createDropDown(
  "System Messages",
  "systemDropDown",
  configUI.panel,
  209,
  -15,
  120,
  dropdownData.SystemAndGameMessages
)
configUI.panel.socialDropDown =
    createDropDown("Social", "socialDropDown", configUI.panel, 359, -15, 120, dropdownData.Social)

configUI.panel.NPCDropDownButton =
    createDropDown("NPC Chatter", "NPCDropDown", configUI.panel, 509, -15, 120, dropdownData.NPCChatter)
--Config Options
...
configUI.panel.selected.Text:SetPoint("BOTTOM", configUI.panel.normalDropDown, "BOTTOM", 90, -40)
...
configUI.panel.selected.description =
    configUI.panel.selected:CreateFontString("description", "OVERLAY", "GameFontNormal")
...
--SettingDropDown
configUI.panel.selected.soundDropDown = createDropDown(
  "Sound",
  "soundDropDown",
  configUI.panel.selected.Text,
  195,
  0,
  120,
  dropdownData.Sounds,
...
configUI.panel.selected.previewButton =
    CreateFrame("Button", "CN_Config_selectedPreviewButton", configUI.panel, "GameMenuButtonTemplate")
configUI.panel.selected.previewButton:SetPoint("CENTER", configUI.panel.selected.soundDropDown, "CENTER", 175, 0)
